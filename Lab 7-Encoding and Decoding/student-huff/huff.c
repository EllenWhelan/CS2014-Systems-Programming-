...
void
make_compound(struct huffchar *new_compound, struct huffchar *smallest, struct huffchar *second_smallest, int seqno) {
...
void huffcoder_print_codes(struct huffcoder *this) {
    ...
    for (i = 0; i < NUM_CHARS; i++) {
        ...
        for (j = this->code_lengths[i] - 1; j >= 0; j--) {
            //buffer[j] = ((this->codes[i] >> j) & 1) + '0'; DG
...
void huffcoder_encode(struct huffcoder *this, char *input_filename,
    ...
    int c;  // we need the character to be
    // unsigned to use it as an index
    FILE *file;
    file = fopen(input_filename, "r");
    assert(file != NULL);
    struct bitfile *temp_bitfile = bitfile_open(output_filename, "w");

    c = fgetc(file);
    while (!feof(file)) {
        unsigned long long char_code = this->codes[c];
        for (int i = this->code_lengths[c]; i > 0; i--) {
            int bit_to_write = (char_code >> (i - 1)) & 1;
            bitfile_write_bit(temp_bitfile, bit_to_write);
        }
        c = fgetc(file);
    }
    unsigned long long char_code = this->codes[4];

    for (int i = this->code_lengths[4]; i > 0; i--) {
        int bit_to_write = (char_code >> (i - 1)) & 1;
        bitfile_write_bit(temp_bitfile, bit_to_write);
    }

    bitfile_close(temp_bitfile);
...
void huffcoder_decode(struct huffcoder *this, char *input_filename,
    ...
    struct bitfile * temp_bitfile = bitfile_open(input_filename, "r");
    temp_bitfile->index=8;
    struct huffchar * root = this->tree;
    int finished=0;

    while (finished==0) {
        while (root->is_compound == 1) {
         int temp_bit = bitfile_read_bit(temp_bitfile);
            if (temp_bit == 0) {
                root = root->u.compound.left;
            } else if (temp_bit == 1) {
                root = root->u.compound.right;
            }
        }
